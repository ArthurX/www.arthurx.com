...
/* if(stristr($text, 'http') === FALSE) {
 ...
 if(stristr($text, 'html') === FALSE) {
   $text="0";
  }
*/


$text = str_replace( "<" , "101" ,$text);
$text = str_replace( "http" , "101" ,$text);
$text = str_replace( "html" , "101" ,$text);
$text = str_replace( "poker" , "101" ,$text);

#checkForHTML($text); put it to a short
 if(stristr($text, '101') === FALSE) {
   $ToMuchHTML = FALSE;
  }
...
// read last input text and cheq whether it's the same                             **********************

$query = "SELECT text ,id  FROM  guestbook  WHERE  id=(SELECT MAX(id) FROM guestbook)";
...
while($row = mysql_fetch_array($result)){
    //echo "The entry with the highest id = ". $row['id']. "<br> Text: ".$row['text'];
    if ( $row['text'] == $text ){$Double_Submission = true ;} else {$Double_Submission = false;}
     echo "<br />";
}



if(isset($_POST) && strlen($text)>5 && $Double_Submission==false && $ToMuchHTML==FALSE ) {


mysql_query("INSERT INTO guestbook (name, email, subject, text) VALUES('$name', '$email', '$subject', '$text' ) ")
or die(mysql_error());

echo "Thank you for writing in my Guestbook.<p>";

//     Send an email to myself                                      ******************
mail( "arthurx@gmail.com", "Submission in guestbook: $subject",
  $text, "From: $email" );
...
//      Create Color Array                                    ****************
$Light_Color_Array = Array( "#41fe1e",  // Grasgreen
"#fa00bc",  // Dark rose Primrose
"#f2fc36",  // Citron yellow
"#29fd7d",   //Gras/Mint Green
"#74f6a8",  // Mint Green
"#fbf943",   // Yellow
"#c536fc");  // Light purple

$Former_Color = sizeof($Light_Color_Array);
//       Start Table
echo "<table>";
...
echo "<input type=\"hidden\"  name=\"Sam\" value=\"DeBam\" /> ";
echo  "<tr bgcolor=blue><td>Name:<input type=\"text\" name=\"name\" MAXLENGTH=\"30\"  >";
echo  "<td colspan=2 > Email:<input type=\"text\" name=\"email\" MAXLENGTH=\"30\"  >";
echo  "<tr><td colspan=3 bgcolor=red>Subject:<input type=\"text\" name=\"subject\" SIZE=\"70\" MAXLENGTH=\"70\" >";
echo "<tr><td>";
Random_Image($Barcelona_Names , 1 , 'Barcelona.html', 'Barcelona') ;
    echo  "<td colspan=2 bgcolor=";
    //   Create Random Color with Cheq that it'snt doulbe
    do {$randomno=rand(0, sizeof($Light_Color_Array)-1 );}
    while ($Former_Color==$randomno);
    $Former_Color=$randomno;
    echo $Light_Color_Array[$randomno] . ">";

echo "Text: <br><textarea name=\"text\"  cols=50 rows=10 MAXLENGTH=\"1000\"  ></textarea>";
echo "<tr bgcolor=black><td colspan=2><td><input type=\"submit\" value=\"Submit\"> ";
echo  "</form>";
echo "<tr bgcolor=black> <td colspan=3 > &nbsp;";
...
$result = mysql_query($query) or die(mysql_error());
// Create output table and results

while($row = mysql_fetch_array($result)){
    echo "<tr bgcolor=blue ><td>Name:&nbsp;" . $row['name'];
    // replace @ by Heart
    $email = str_replace( "@" , "<img src=\"Heart_for_Email.png\">bla@" ,$row['email']);
    echo " <td>". $email;
    echo " <td align=right>Date: ". $row['date']."<tr><td colspan=3 bgcolor=red>Subject: ". $row['subject']. " <tr><td> " ;
    Random_Image($Barcelona_Names , 1 , 'Barcelona.html', 'Barcelona') ;
    echo  "<td colspan=2 bgcolor=";
    //   Create Random Color with Cheq that it'snt doulbe
    do {$randomno=rand(0, sizeof($Light_Color_Array)-1 );}
